...
public abstract class FocusRequest extends BaseBizRequest<POListData<POFocus>> {
    ...
    public String getPath() {
        return BaseAPI.Path.newHomePage_work_list;
    }
    ...
    public void onRequestResult(String result) {
        Type type = new TypeToken<POCommonResp<POListData<POFocus>>>() {
        }.getType();
        responseBean = new Gson().fromJson(result, type);
    }
    POListData<POFocus> listP;
    @Override
    public void startRequest(Map<String, String> params) {
        listP = new POListData<>();
        listP.setNext_cursor(10);
        List<POFocus> poFocusList = new ArrayList<>();
        POFocus poFocus1 = new POFocus();
        if(type ==FOCUS){
